    ...
    public void action(ContextMenuEvent event) {
        ...
        Query newQuery =
                new Query(
                        (Map<String, List<String>>)
                                HttpUtil.updateJsonValuesByMap(request.getQuery(), payload));
        ...
        if (contentType == ContentType.FORM) {
            Form newForm =
                    new Form(
                            (Map<String, List<String>>)
                                    HttpUtil.updateJsonValuesByMap(
                                            Form.of(new String(request.getContent())), payload));
            request.setContent(newForm.toRaw().getBytes());
        } else if (contentType == ContentType.JSON) {
            JsonElement jsonElement =
                    JsonParser.fromJsonStr(new String(request.getContent()), JsonElement.class);
