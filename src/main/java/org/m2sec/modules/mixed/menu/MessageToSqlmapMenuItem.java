...
public class MessageToSqlmapMenuItem extends AbstractMenuItem {
    ...
    public String displayName() {
        return "Message To SqlMap";
    }

    @Override
    public boolean isDisplay(ContextMenuEvent event) {
        return !GalaxyMain.config.getMixed().getSqlmap().getPath().isEmpty()
    ...
    public void action(ContextMenuEvent event) {
        ...
        try (FileWriter fileWriter = new FileWriter(tmpFilePath)) {
            fileWriter.write(new String(request.toMessage()));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        MixedConfig.SqlMapConfig sqlMapConfig = GalaxyMain.config.getMixed().getSqlmap();
    ...
    private static ProcessBuilder getProcessBuilder(String cmd) {
        ...
        if (os.contains("win")) {
            ...
            processBuilder =
                    new ProcessBuilder(
                            "cmd", "/c", "start", "cmd", "/c", winCommand.formatted(cmd, cmd));
