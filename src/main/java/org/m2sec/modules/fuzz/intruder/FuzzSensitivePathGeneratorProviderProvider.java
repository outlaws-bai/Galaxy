...
public class FuzzSensitivePathGeneratorProviderProvider extends AbstractPayloadGeneratorProvider {
    ...
    public static Set<String> generatePayloadSet(String originPath, boolean isBypass) {
        ...
        for (String sensitivePath : FileUtil.readFileAsStringArray(Constants.FUZZ_SENSITIVE_PATH_DICT_FILE_PATH)) {
            payloads.add(sensitivePath);
            if (parts.length > 2) {
                for (int i = 1; i < parts.length - 1; i++) {
                    String partSensitivePath = String.join("/", Arrays.copyOfRange(parts, 0, i + 1)) + sensitivePath;
                    if (isBypass) {
