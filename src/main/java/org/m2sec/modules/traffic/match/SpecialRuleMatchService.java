...
public class SpecialRuleMatchService {
    ...
    public static void matchRequest(
            Request request, int messageId, HttpTrafficAutoModificationConfig.MatchConfig matchConfig) {
    ...
    public static Annotations matchResponse(
            Response response, int messageId, HttpTrafficAutoModificationConfig.MatchConfig matchConfig) {
        Tuple<Integer, Set<String>> temp =
                MATCHED.containsKey(messageId)
                        ? MATCHED.get(messageId)
                        : new Tuple<>(0, new HashSet<>());
        ...
        for (Map.Entry<String, Integer> entry :
                matchConfig.getResponseContentMatches().entrySet()) {
        ...
        if (!temp.getSecond().isEmpty()) {
            sb.append("requestParamMatchedKeys: ")
                    .append(String.join(",", temp.getSecond()))
                    .append("\r\n");
        }
        if (!responseHeaderMatchedKeys.isEmpty()) {
            sb.append("responseHeaderMatchedKeys: ")
                    .append(String.join(",", responseHeaderMatchedKeys))
                    .append("\r\n");
        }
        if (!responseContentMatchedKeys.isEmpty()) {
            sb.append("responseContentMatchedKeys: ")
                    .append(String.join(",", responseContentMatchedKeys))
                    .append("\r\n");
        }
