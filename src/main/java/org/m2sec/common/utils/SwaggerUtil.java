...
public class SwaggerUtil {
    ...
    public static List<ApiInfo> extractApiInfoFromSwagger(String docs) {
        ...
        String swaggerVersion =
                jsonObject.has("swagger") ? jsonObject.get("swagger").getAsString() : null;
        String openApiVersion =
                jsonObject.has("openapi") ? jsonObject.get("openapi").getAsString() : null;
    ...
    private static List<ApiInfo> extractV2(JsonObject swaggerObject) {
        ...
        // 获取引用对象
        JsonObject refs =
                swaggerObject.has("definitions")
                        ? swaggerObject.getAsJsonObject("definitions")
                        : new JsonObject();
        // 解析path结构
                    ...
                    if (methodObject != null) {
                        ...
                        if (methodObject.has("consumes")
                            ...
                            contentTypeHeaderValue =
                                    methodObject.getAsJsonArray("consumes").get(0).getAsString();
                        ContentType contentType =
                                HttpUtil.getContentType(methodStr, contentTypeHeaderValue);
                        ...
                        if (methodObject.has("tags") && methodObject.get("tags").isJsonArray()) {
                            ...
                            for (int i = 0; i < methodTagNames.size(); i++) {
                                ...
                                apiInfo.getNotes()
                                        .put(
                                                "tag-description-" + i,
                                                tags.getOrDefault(tagName, ""));
                            }
                        }
                        if (methodObject.has("summary") && methodObject.get("summary") != null)
                            apiInfo.getNotes()
                                    .put("summary", methodObject.get("summary").getAsString());
                        if (methodObject.has("description")
                                && methodObject.get("description") != null)
                            apiInfo.getNotes()
                                    .put(
                                            "description",
                                            methodObject.get("description").getAsString());
                        if (methodObject.has("operationId")
                                && methodObject.get("operationId") != null)
                            apiInfo.getNotes()
                                    .put(
                                            "operationId",
                                            methodObject.get("operationId").getAsString());
                        // 处理参数
                        if (methodObject.has("parameters"))
                            methodObject
                                    ...
                                    .forEach(
                                            (parameter) ->
                                                    updateApiInfoByParameters(
                                                            apiInfo,
                                                            parameter.getAsJsonObject(),
                                                            refs));
                    }
                }
            }
    ...
    private static List<ApiInfo> extractV3(JsonObject swaggerObject) {
        ...
        // 获取引用对象
        JsonObject refs =
                swaggerObject.has("components")
                        ? swaggerObject.getAsJsonObject("components")
                        : new JsonObject();
        // 解析path结构
        if (swaggerObject.has("paths") && swaggerObject.get("paths") != null) {
            JsonObject pathsObject = swaggerObject.getAsJsonObject("paths");
                ...
                for (String methodStr : pathObject.keySet()) {
                    ...
                    if (methodObject != null) {
                        ...
                        JsonObject requestBodyJsonObject =
                                methodObject.has("requestBody")
                                        ? methodObject.getAsJsonObject("requestBody")
                                        : new JsonObject();
                        JsonObject contentJsonObject =
                                requestBodyJsonObject.has("content")
                                        ? requestBodyJsonObject.getAsJsonObject("content")
                                        : new JsonObject();
                        if (!contentJsonObject.isJsonNull()) {
                            if (contentJsonObject.keySet().contains("application/json"))
                                ...
                                contentTypeHeaderValue =
                                        contentJsonObject.keySet().iterator().next();
                        }
                        ContentType contentType =
                                HttpUtil.getContentType(methodStr, contentTypeHeaderValue);
                        ApiInfo apiInfo = new ApiInfo(version, method, path, contentType);
                        ...
                        if (methodObject.has("tags") && methodObject.get("tags") != null) {
                            apiInfo.getNotes()
                                    .put("tags", methodObject.getAsJsonArray("tags").toString());
                        }
                        if (methodObject.has("summary") && methodObject.get("summary") != null) {
                            apiInfo.getNotes()
                                    .put("summary", methodObject.get("summary").getAsString());
                        ...
                        if (contentTypeHeaderValue != null) {
                            if (contentJsonObject.has(contentTypeHeaderValue)
                                    && contentJsonObject
                                            .get(contentTypeHeaderValue)
                                            .isJsonObject()) {
                                JsonObject bodyJsonObject =
                                        contentJsonObject.getAsJsonObject(contentTypeHeaderValue);
    ...
    private static void updateApiInfoByParameters(
            ApiInfo apiInfo, JsonObject parameter, JsonObject refs) {
        ...
        String description =
                parameter.has("description") ? parameter.get("description").getAsString() : null;
        String type = parameter.has("type") ? parameter.get("type").getAsString() : null;
        if (type == null
                && parameter.has("schema")
                && parameter.get("schema") != null
                && apiInfo.getVersion() == ApiInfo.Version.V3) {
        ...
        switch (position) {
            case "path" ->
                    apiInfo.setPath(
                            apiInfo.getPath()
                                    .replace("{" + name + "}", getValueByType(type).toString()));
        ...
        if (description != null)
            apiInfo.getNotes()
                    .put(
                            "parameter-description-" + position + "-" + name + "-" + type,
                            description);
    }

    private static void updateApiInfoByRef(
            ApiInfo apiInfo, JsonObject jsonObject, JsonObject refs, boolean isArray) {
    ...
    private static HashMap<String, Object> getRequestByModel(
            ApiInfo apiInfo, JsonObject model, JsonObject refs, String key) {
        ...
        JsonObject properties =
                model.has("properties") ? model.get("properties").getAsJsonObject() : null;
        ...
        for (String propertyName : properties.keySet()) {
            ...
            if (refStr != null) {
                ...
                if (subModel != null)
                    retVal.put(propertyName, getRequestByModel(apiInfo, subModel, refs, refStr));
            } else {
                JsonArray enums =
                        property.has("enum") ? property.get("enum").getAsJsonArray() : null;
                String description =
                ...
                String example =
                        property.has("example") ? property.get("example").getAsString() : null;
                if (description != null)
                    apiInfo.getNotes()
                            .put("model-description-body-" + key + "-" + subType, description);
                if (enums != null) {
    ...
    private static void updateApiInfoByRequestBody(
            ApiInfo apiInfo, JsonObject requestBody, JsonObject refs) {
        if (requestBody.has("schema") && requestBody.get("schema").isJsonObject()) {
            JsonObject schemaObject = requestBody.getAsJsonObject("schema");
            String schemaType =
                    schemaObject.has("type") ? schemaObject.get("type").getAsString() : null;
            if (schemaType == null) {
                updateApiInfoByRef(apiInfo, schemaObject, refs, false);
            } else if (schemaType.equals("array")) {
                JsonObject itemsObject =
                        schemaObject.has("items") ? schemaObject.getAsJsonObject("items") : null;
                if (itemsObject != null) {
