    ...
    public static String getIntruderWrappedText(byte[] data) {
        ...
        if (startIndex == -1) {
            throw new RuntimeException(
                    "cannot find first intruder flag in data. " + Arrays.toString(data));
        }
        int endIndex =
                ByteUtil.findFirstCharIndex(data, BURP_INTRUDER_VAR_FLAG, startIndex + 1);
        if (endIndex == -1) {
            throw new RuntimeException(
                    "cannot find second intruder flag in data. " + Arrays.toString(data));
        }
