...
public class ByteUtil {
    ...
    public static byte[] replaceBytes(
            byte[] original, int startIndex, int endIndex, byte[] replacement) {
        if (startIndex < 0 || endIndex > original.length || startIndex > endIndex) {
            throw new IllegalArgumentException(
                    String.format(
                            "Invalid startIndex or endIndex index. original: %s, startIndex: %d, endIndex: %d, replacement: %s",
    ...
    public static byte[] subBytes(byte[] originalArray, int startIndex, int endIndex) {
        ...
        if (startIndex < 0 || endIndex - 1 >= originalArray.length || startIndex > endIndex - 1) {
            throw new IllegalArgumentException(
                    String.format(
                            "Invalid startIndex or endIndex. originalArray: %s, startIndex: %d, endIndex: %d",
                            Arrays.toString(originalArray), startIndex, endIndex));
        }
