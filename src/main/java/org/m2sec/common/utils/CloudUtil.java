...
public class CloudUtil {
    ...
    private static final List<String> SIGNED_HEADERS_BLACKLIST =
            new ArrayList<>(List.of("expect", "user-agent", "connection"));
    ...
    public static void signAws(Request request, CloudConfig.AwsConfig awsConfig) {
        ...
        String contentSha256 =
                HashUtil.calcHashToHex(request.getContent(), DigestAlgorithm.SHA_256);
        // calc authorization
        headers.add("x-amz-date", amzDate).add("x-amz-content-sha256", contentSha256);
        // add some header
        String method = request.getMethod();
        if (!(method.equals(Method.GET.name())
                || method.equals(Method.OPTIONS.name())
                || method.equals(Method.HEAD.name()))) {
            headers.put(
                    "content-md5",
                    HashUtil.calcHashToBase64(request.getContent(), DigestAlgorithm.MD5));
            headers.put(
                    Constants.HTTP_HEADER_CONTENT_TYPE, ContentType.FORM.getHeaderValuePrefix());
        }
        if (awsConfig.getToken() != null && !awsConfig.getToken().isBlank())
            headers.add("x-amz-security-token", awsConfig.getToken());
        String authorization =
                calcAwsAuthorization(
                        awsConfig,
                        amzDate,
                        contentSha256,
                        request.getQuery(),
    ...
    private static String calcAwsAuthorization(
            CloudConfig.AwsConfig awsConfig,
            String amzDate,
            String contentSha256,
            Query query,
            Headers headers,
        ...
        String stringToSign =
                "AWS4-HMAC-SHA256\n"
                        ...
                        + scopeString
                        + "\n"
                        + HashUtil.calcHashToHex(
                                canonicalRequest.getBytes(), DigestAlgorithm.SHA_256);
        ...
        byte[] kRegion =
                HashUtil.calcHmac(kDate, DigestAlgorithm.SHA_256, awsConfig.getRegion().getBytes());
        byte[] kService =
                HashUtil.calcHmac(
                        kRegion, DigestAlgorithm.SHA_256, awsConfig.getService().getBytes());
        byte[] kSignature =
                HashUtil.calcHmac(kService, DigestAlgorithm.SHA_256, AWS_4_REQUEST.getBytes());
        String signature =
                HashUtil.calcHmacToHex(
                        kSignature, DigestAlgorithm.SHA_256, stringToSign.getBytes());
        // calc authorization
        return String.format(
                "AWS4-HMAC-SHA256 Credential=%s, SignedHeaders=%s, Signature=%s",
                awsConfig.getAk() + "/" + scopeString, signedHeaderNames, signature);
    }
}
