...
public class HashUtil {
    ...
    public static byte[] calcHash(byte[] data, DigestAlgorithm digestAlgorithm) {
        if (GalaxyMain.env.equals(OperatingEnv.LOCAL)) {
            try {
                MessageDigest md =
                        MessageDigest.getInstance(digestAlgorithm.toString().replace("_", ""));
                return md.digest(data);
            } catch (NoSuchAlgorithmException e) {
                throw new RuntimeException(e);
            }
        }
        return GalaxyMain.burpApi
                .utilities()
                .cryptoUtils()
                .generateDigest(ByteArray.byteArray(data), digestAlgorithm)
                .getBytes();
    }
    ...
    public static String calcHmacToHex(
            byte[] secret, DigestAlgorithm digestAlgorithm, byte[] data) {
