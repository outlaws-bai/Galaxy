...
public class Log {
    ...
    private String formatMessage(String level, String format, Object... args) {
        ...
        String result;
        try {
            result = String.format("[%s: %s] [%s]: %s", formattedTime, level, this.moduleName, String.format(format,
                args));
        } catch (MissingFormatArgumentException e) {
            safeError("format error. e: " + e.getMessage() + " , format: " + format + ", args: " + Arrays.toString(args));
            result = String.format("[%s: %s] [%s]: %s", formattedTime, level, this.moduleName, format);
        }
