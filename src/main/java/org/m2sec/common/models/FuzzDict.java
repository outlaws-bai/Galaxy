...
public class FuzzDict {
    ...
    public static FuzzDict of(List<Tuple<Request, Response>> requestResponses) {
        ...
        Set<String> params =
                requestResponses.stream()
                        .flatMap(x -> getParamsNames(x).stream())
                        .collect(Collectors.toSet());
        Set<String> headers =
                requestResponses.stream()
                        .map(Tuple::getFirst)
                        .flatMap(request -> request.getHeaders().keySet().stream())
                        .collect(Collectors.toSet());
        Set<String> cookies =
                requestResponses.stream()
                        .map(Tuple::getFirst)
                        .map(Request::getCookies)
                        .filter(Objects::nonNull)
                        .map(Cookies::keySet)
                        .flatMap(Set::stream)
                        .collect(Collectors.toSet());
    ...
    public static void extractParamNamesFromJsonObject(Set<String> params, JsonObject jsonObject) {
        for (String key : jsonObject.keySet()) {
            ...
            if (jsonElement.isJsonObject()) {
                extractParamNamesFromJsonObject(
                        params, jsonElement.getAsJsonObject()); // 递归处理内部的JsonObject
            } else if (jsonElement.isJsonArray()) {
                extractParamNamesFromJsonArray(params, jsonElement.getAsJsonArray());
            }
        }
