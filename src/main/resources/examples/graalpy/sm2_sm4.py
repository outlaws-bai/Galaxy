...
def hook_request_to_burp(request: Request) -> Request:
    ...
    # 调用内置函数解密，拿到sm4密钥
    key: bytes = asymmetric_decrypt(encryptedKey, privateKey1)
    # 调用内置函数解密报文
...
def hook_request_to_server(request: Request) -> Request:
    ...
    encryptedData: bytes = symmetric_encrypt(data, sm4Secret)
    # 调用内置函数加密sm4Secret
    encryptedKey: bytes = asymmetric_encrypt(sm4Secret, publicKey1)
...
def hook_response_to_burp(response: Response) -> Response:
    ...
    encryptedKey: bytes = get_key(response.getContent())
    # 调用内置函数解密，拿到sm4密钥
    key: bytes = asymmetric_decrypt(encryptedKey, privateKey2)
    # 调用内置函数解密报文
    data: bytes = symmetric_decrypt(encryptedData, key)
    # 更新body
...
def hook_response_to_client(response: Response) -> Response:
    ...
    encryptedData: bytes = symmetric_encrypt(data, sm4Secret)
    # 调用内置函数加密sm4Secret
    encryptedKey: bytes = asymmetric_encrypt(sm4Secret, publicKey2)
    # 将已加密的数据转换为Server可识别的格式
    body: bytes = to_data(encryptedData, encryptedKey)
    # 更新body
