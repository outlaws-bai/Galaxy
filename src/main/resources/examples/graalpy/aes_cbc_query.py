...
from java.org.m2sec.core.models import Request, Response, Query
...
def hook_request_to_burp(request: Request) -> Request:
    ...
    """
    # 获取需要解密的数据
    encryptedData: bytes = CodeUtil.b64decode(request.getQuery().getFirst("username"))
    # 调用函数解密
    data: bytes = decrypt(encryptedData)
    # 更新query
    newQuery = Query()
    newQuery.put("username", convert_bytes(data).decode())
...
def hook_request_to_server(request: Request) -> Request:
    ...
    """
    # 获取被解密的数据
    data: bytes = request.getQuery().getFirst("username").encode()
    # 调用函数加密回去
    ...
    # 更新query
    newQuery = Query()
    newQuery.put("username", CodeUtil.b64encodeToString(encryptedData))
    request.setQuery(newQuery)
    return request
