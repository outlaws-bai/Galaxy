...
from java.org.m2sec.core.models import Request, Response, FormData
...
def hook_request_to_burp(request: Request) -> Request:
    ...
    """
    # 获取需要解密的数据
    encryptedData: bytes = CodeUtil.b64decode(request.getFormData().getFirst("username"))
    # 调用函数解密
    data: bytes = decrypt(encryptedData)
    # 更新FormData
    newFormData = FormData()
    newFormData.put("username", convert_bytes(data).decode())
    request.setFormData(newFormData)
    return request
...
def hook_request_to_server(request: Request) -> Request:
    ...
    """
    # 获取被解密的数据
    data: bytes = request.getFormData().getFirst("username").encode()
    # 调用函数加密回去
    ...
    # 更新FormData
    newFormData = FormData()
    newFormData.put("username", CodeUtil.b64encodeToString(encryptedData))
    request.setFormData(newFormData)
    return request
