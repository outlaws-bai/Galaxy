...
def hook_request_to_server(request: Request) -> Request:
    ...
    encryptedData: bytes = symmetric_encrypt(data, aesSecret)
    # 调用内置函数加密aesSecret
    encryptedKey: bytes = asymmetric_encrypt(aesSecret, publicKey1)
...
def hook_response_to_client(response: Response) -> Response:
    ...
    encryptedData: bytes = symmetric_encrypt(data, aesSecret)
    # 调用内置函数加密aesSecret
    encryptedKey: bytes = asymmetric_encrypt(aesSecret, publicKey2)
    # 将已加密的数据转换为Server可识别的格式
    body: bytes = to_data(encryptedData, encryptedKey)
    # 更新body
