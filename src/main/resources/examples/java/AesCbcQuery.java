...
public class AesCbcQuery {
    ...
    public Request hookRequestToBurp(Request request) {
        // 获取需要解密的数据
        byte[] encryptedData = CodeUtil.b64decode(request.getQuery().getFirst("username"));
        // 调用函数解密
        byte[] data = decrypt(encryptedData);
        // 更新query
        Query newQuery = new Query();
        newQuery.put("username", new String(data));
    ...
    public Request hookRequestToServer(Request request) {
        // 获取被解密的数据
        byte[] data = request.getQuery().getFirst("username").getBytes();
        // 调用函数加密回去
