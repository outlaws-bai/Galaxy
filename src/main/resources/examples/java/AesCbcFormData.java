...
public class AesCbcFormData {
    ...
    public AesCbcFormData(Logger log) {
        this.log = log;
    }
    ...
    public Request hookRequestToBurp(Request request) {
        // 获取需要解密的数据
        byte[] encryptedData = CodeUtil.b64decode((String) request.getFormData().getFirst("username"));
        // 调用函数解密
        byte[] data = decrypt(encryptedData);
        // 更新FormData
        FormData newFormData = new FormData();
        newFormData.put("username", new String(data));
    ...
    public Request hookRequestToServer(Request request) {
        // 获取被解密的数据
        byte[] data = ((String)request.getFormData().getFirst("username")).getBytes();
        // 调用函数加密回去
