...
var FormData = Java.type("org.m2sec.core.models.FormData")
...
function hook_request_to_burp(request) {
    // 获取需要解密的数据
    encryptedData = CodeUtil.b64decode(request.getFormData().getFirst("username"))
    // 调用函数解密
    data = decrypt(encryptedData)
    // 更新FormData
    newFormData = new FormData();
    newFormData.put("username", byteArrayToString(data));
...
function hook_request_to_server(request) {
    // 获取被解密的数据
    data = stringToByteArray(request.getFormData().getFirst("username"))
    // 调用函数加密回去
