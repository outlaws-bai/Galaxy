...
var Query = Java.type("org.m2sec.core.models.Query")
...
function hook_request_to_burp(request) {
    // 获取需要解密的数据
    encryptedData = CodeUtil.b64decode(request.getQuery().getFirst("username"))
    // 调用函数解密
    data = decrypt(encryptedData)
    // 更新query
    newQuery = new Query();
    newQuery.put("username", byteArrayToString(data));
...
function hook_request_to_server(request) {
    // 获取被解密的数据
    data = stringToByteArray(request.getQuery().getFirst("username"))
    // 调用函数加密回去
